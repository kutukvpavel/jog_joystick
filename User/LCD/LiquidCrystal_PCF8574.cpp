...
void LiquidCrystal_PCF8574::clear()
{
  ...
  _send(0x01);
  vTaskDelay(pdMS_TO_TICKS(2)); // this command takes 1.5ms!
} // clear()
...
void LiquidCrystal_PCF8574::print(const char* str)
{
  char c;
  while (c = *str++)
  {
...
void LiquidCrystal_PCF8574::_writeNibble(uint8_t halfByte, bool isData)
{
  ...
  i2c::write_byte(_i2cAddr, data | _enable_mask);
  delay_us(1); // enable pulse must be >450ns
  i2c::write_byte(_i2cAddr, data);
  delay_us(37); // commands need > 37us to settle
} // _writeNibble
